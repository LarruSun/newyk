  ...
  <div class="contain">
    <div class="common-title">
      <div>
        ...
        <i class="el-icon-arrow-right"></i>
        <span class="active-title">【积分兑券】券列表</span>
      </div>
    </div>
      ...
      <el-table :data="tableData" border :height="tableHeight" class="table">
        ...
        <el-table-column label="操作" align="center" width="200">
          <template slot-scope="scope">
            ...
            <el-button size="small" @click="remove(scope.row.integerExchangeId)" type="danger">移除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    remove(id){
      this.$confirm('确定要移除此积分兑券数据吗？','提示',{
        'type' : 'warning'
      }).then(res => {
        this.$http.post('newcoupon/newcouponSev/edit/updateIntegralExchangeStatu',{
    ...
    getList(){
      ...
      this.$http.post('newcoupon/newcouponSev/get/getCouponIntegralPutList',this.filterParams(this.params)).then(res => {
        setTimeout(()=>{
